...
// Função para truncar texto da bio
const truncateBio = (text: string, maxLength: number = MAX_BIO_LENGTH) => {
...
const bioHeaderStyle: React.CSSProperties = {
    display: "flex",
    alignItems: "center",
    gap: "6px",
    marginBottom: "6px",
...
const bioIndicatorStyle: React.CSSProperties = {
    width: "3px",
    height: "12px",
...
const bioButtonStyle: React.CSSProperties = {
    display: "flex",
    alignItems: "center",
    gap: "4px",
    fontSize: "9px",
...
export function getListMembers(Component): ComponentType {
    ...
    return (props) => {
        ...
        const [bioExpanded, setBioExpanded] = useState<{ [key: string]: boolean }>({})
                ...
                <div style={containerStyle}>
                    {paginatedMembers.map((member) => {
                        try {
                            ...
                            const memberBio = member.info?.bio || ""
                            const isBioExpanded = bioExpanded[member.id] || false
                            const shouldShowButton = memberBio.length > MAX_BIO_LENGTH
                            const displayText = isBioExpanded ? memberBio : truncateBio(memberBio)
                                ...
                                <div key={member.id} style={{ position: "relative" }}>
                                    <Component
                                        ...
                                        whatsapp={
                                            ...
                                            member.info?.phone &&
                                            String(
                                                member.info?.social_links?.whatsapp
                                            ) === "1"
                                    ...
                                    {/* Bio renderizada diretamente */}
                                    {memberBio && (
                                        <div style={bioContainerStyle}>
                                            <div style={bioHeaderStyle}>
                                                <div style={bioIndicatorStyle}></div>
                                                <span style={bioLabelStyle}>BIO</span>
                                            </div>
                                            ...
                                            {shouldShowButton && (
                                                ...
                                                >
                                                    ...
                                                    <span>{isBioExpanded ? 'Ver menos' : 'Ver mais'}</span>
                                                </button>
                                            )}
                                        </div>
                                    )}
